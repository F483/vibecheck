...
The labels are whatever strings that person uses. There is no built-in genre
taxonomy, and the app never interprets a label — it only learns which sounds go
with which string.
...
Measurements come from the reference collection described in §3.

## 1. Idea

A local tool that learns a user's labelling of music and pre-labels new
tracks with it.
...
Example vocabulary, used as the reference throughout this document: 24 labels
— 8 colours for feel x 3 levels for how much the user likes the track, written
as strings like `Red_High`. The app never knows any of that structure; it sees
24 opaque symbols. Another user's vocabulary will look nothing like this, and
...
- Audio only — no metadata signals (artist/album/year/filename). **decided**
  Artist is probably the strongest predictor of any user's labels; it is
  excluded deliberately so the model learns sound, not lookup. Revisit later.
...
- **Local-first**: runs entirely on a laptop-class machine, offline, no upload of
  ...
  **decided**
- **Portable**: must work on any user's mac and any user's collection.
...
- The reference vocabulary is `Colour_Level` over 8 colours and 3 levels, where
  **A is best** — hence the skew: **C 65%, B 30%, A 5%**. Few favourites, many
  merely-fine tracks.
...
Not app behaviour — realistic expectation-setting for the reference vocabulary,
whose labels mix a "sound" component with a "how much do I like this" component.
The second is the risky part:
...
implement 2-3 behind one interface, measure on the user's own library, keep
the winner.
The MFCC baseline is mandatory — if a 200 MB neural model doesn't clearly beat
it on real labels, it isn't worth carrying.
...
- Which embedding backend wins on real labels, at what wall-clock and install
  cost.
...
Open for the maintainer, but not blocking:
- Ceiling check: relabel ~100 tracks blind and compare against the earlier
  labels. That self-agreement rate is the real accuracy target; chasing above
  it is chasing noise.
